...
This single command:
1. ✅ Verifies Clojure is installed
2. ✅ Checks proxy shim script exists
3. ✅ Starts proxy shim (handles JWT authentication)
4. ✅ Downloads ALL dependencies through shim
5. ✅ Runs all 97 tests
6. ✅ Tests server compilation
7. ✅ Stops proxy shim
...
```
Java/Maven → localhost:15080 (no auth) → Proxy Shim → Claude Security Proxy (JWT) → Maven Central
```
...
Expected output:
```
🔍 Verifying Claude Code sandbox setup...

1️⃣  Checking Clojure installation...
   ✅ Clojure CLI version 1.12.3.1577

2️⃣  Checking proxy shim script...
   ✅ Proxy shim script found

3️⃣  Checking proxy configuration...
   ✅ Proxy configuration exists

4️⃣  Testing proxy shim startup...
   ✓ Proxy shim started on 127.0.0.1:15080
   ✓ Proxy shim stopped
   ✅ Proxy shim startup tested

✅ Claude Code sandbox setup verified!
```
...
Expected output:
```
🧪 Running tests in Claude Code sandbox...
...
**What you'll see:**
```
❌ Clojure not found - run: make claude-code-web-setup
```
...
5. **Is the proxy shim starting?**
   Look for: `✓ Proxy shim started on 127.0.0.1:15080`
...
✅ `make claude-sandbox` completes without errors
✅ All 97 tests pass
✅ Server compiles successfully
✅ `~/.m2/repository` contains 150+ JARs
✅ Subsequent test runs complete in <10 seconds
...
This:
- ✅ Installs Clojure (if needed)
- ✅ Downloads all dependencies (first time only)
- ✅ Runs all tests
- ✅ Verifies server compilation
...
**TL;DR**: Run `make claude-sandbox` and everything just works! 🎉
