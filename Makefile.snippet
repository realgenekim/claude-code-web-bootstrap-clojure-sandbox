...
install-clojure:
	...
	@if command -v clojure >/dev/null 2>&1; then \
		...
		(sudo ./linux-install.sh 2>/dev/null || \
			(echo ""; \
			 echo "⚠️  System-wide installation requires sudo. Manual installation options:"; \
			 echo ""; \
			 echo "Option 1: Run with sudo manually:"; \
			 echo "  sudo ./linux-install.sh"; \
			 echo ""; \
			 echo "Option 2: Install to user directory (~/.local):"; \
			 echo "  mkdir -p ~/.local && ./linux-install.sh --prefix ~/.local"; \
			 echo "  export PATH=~/.local/bin:\$$PATH"; \
			 echo ""; \
			 echo "Option 3: Use asdf version manager:"; \
			 echo "  asdf plugin add clojure"; \
			 echo "  asdf install clojure latest"; \
			 echo "  asdf global clojure latest"; \
			 echo "")) && \
		(command -v clojure >/dev/null 2>&1 && rm linux-install.sh && clojure --version || true); \
	fi
...
claude-sandbox-verify:
	@echo "🔍 Verifying Claude Code sandbox setup..."
	@echo ""
	@echo "1️⃣  Checking Clojure installation..."
	@command -v clojure >/dev/null 2>&1 && \
		echo "   ✅ Clojure $$(clojure --version 2>&1 | head -1)" || \
		(echo "   ❌ Clojure not found - run: make claude-code-web-setup" && exit 1)
	@echo ""
	@echo "2️⃣  Checking proxy shim script..."
	@[ -f script/proxy_shim.py ] && \
		echo "   ✅ Proxy shim script found" || \
		(echo "   ❌ Proxy shim not found" && exit 1)
	@echo ""
	@echo "3️⃣  Checking proxy configuration..."
	@[ -f .proxy-java-shim.env ] && \
		echo "   ✅ Proxy configuration exists" || \
		(echo "   ❌ .proxy-java-shim.env not found" && exit 1)
	@echo ""
	@echo "4️⃣  Testing proxy shim startup..."
	@$(MAKE) proxy-shim-start && sleep 1 && $(MAKE) proxy-shim-stop
	@echo ""
	@echo "✅ Claude Code sandbox setup verified!"
...
claude-sandbox-tests:
	@echo "🧪 Running tests in Claude Code sandbox..."
	@echo "   (Starting proxy shim automatically)"
	@echo ""
	@$(MAKE) runtests-shim
	@echo ""
	@echo "✅ Tests completed in Claude Code sandbox!"

# Run complete Claude sandbox workflow (verify + tests)
.PHONY: claude-sandbox
claude-sandbox:
	@echo "🎯 Running complete Claude Code sandbox workflow..."
	@echo ""
	...
	@echo ""
	@echo "🎉 All Claude Code sandbox checks passed!"
